...
- **Aira Implementation Plan Draft Pattern ([Aira Plan Draft Pattern.md](e:/NAST/Dazzling/GAS/.gemini/memory/Aira Plan Draft Pattern.md))**: Defines the strict architectural, infrastructure, and domain rules for drafting technical implementation plans and refactoring proposals. This plan pattern contains rules (N1 to N8 and D1 to D6) governing positional signatures, explicit facts vs. assumptions, GAS execution boundaries, performance regression benchmarks, contract-to-seat model separation, polymorphic discriminators, LIFO rollbacks, mutation manifest/presentation compliance, and production-ready code standards.
- **Production-Ready Code Standard & Quality Decree ([production-ready-code.md](e:/NAST/Dazzling/GAS/.agents/rules/production-ready-code.md))**: Defines mandatory engineering standards across DazzlingDB and SheetDB covering explicit JSDoc contracts, domain custom exception handling, fail-fast boundary validation, structured context logging without PII leakage, in-memory 2D array batch persistence, and declarative strategy pattern adoption.
