...
function runRelationTests() {
  ...
  console.log("🚀 Initializing SheetDB for Relational Tests...");
  const db = SheetDB.init(REL_TEST_FOLDER_ID, schema);
  ...
  try {
    // 1. SEED DATA
    seedRelationalData(db, testSid);
    pause("SEEDING COMPLETE: 1 Student, 1 Address, and 2 Enrollments created.\n\nPlease verify these exist across the 'Student' and 'Address' sheets.");

    // 2. TEST: Student -> Address (1:1 / hasOne)
    testOneToOneRelation(db, testSid);

    // 3. TEST: Student -> Enrollments (1:N / hasMany)
    testOneToManyRelation(db, testSid);

    // 4. TEST: Reverse Relation (belongsTo)
    testReverseRelation(db, testSid);

    // 5. TEST: Nested MongoDB Style Insert (insertOne)
    testNestedInsertOne(db);

    console.log("\n✅ ALL RELATIONAL SCENARIOS PASSED!");

  } catch (e) {
    console.error("\n❌ RELATION TEST FAILED: " + e.message);
